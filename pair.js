...
export default router;    // Use the international number format (E.164, without '+')
    num = phone.getNumber('e164').replace('+', '');
    ...
    async function initiateSession() {
        ...
        try {
            ...
            KnightBot.ev.on('connection.update', async (update) => {
                ...
                if (connection === 'open') {
                    console.log("✅ 𝐂𝐨𝐧𝐧𝐞𝐜𝐭𝐞𝐝 𝐬𝐮𝐜𝐜𝐞𝐬𝐬𝐟𝐮𝐥𝐥𝐲!");
                    console.log("📱 𝐒𝐞𝐧𝐝𝐢𝐧𝐠 𝐬𝐞𝐬𝐬𝐢𝐨𝐧 𝐟𝐢𝐥𝐞 𝐭𝐨 𝐙𝐞𝐫𝐨𝐛𝐮𝐠 𝐮𝐬𝐞𝐫...");
                    ...
                    try {
                        ...
                        console.log("📄 𝐒𝐞𝐬𝐬𝐢𝐨𝐧 𝐟𝐢𝐥𝐞 𝐬𝐞𝐧𝐭 𝐬𝐮𝐜𝐜𝐞𝐬𝐬𝐟𝐮𝐥𝐥𝐲");
...
└─────────────────┈•\n\n`
            ...
            if (!Zerobug.authState.creds.registered) {
                await delay(3000); // Wait 3 seconds before requesting pairing code
                num = num.replace(/[^\d+]/g, '');
                if (num.startsWith('+')) num = num.substring(1);

                try {
                    let code = await Zerobug.requestPairingCode(num);
                    code = code?.match(/.{1,4}/g)?.join('-') || code;
                    if (!res.headersSent) {
                        console.log({ num, code });
                        await res.send({ code });
                    ...
                    console.error('Error requesting pairing code:', error);
                    if (!res.headersSent) {
                        res.status(503).send({ code: '𝐅𝐚𝐢𝐥𝐞𝐝 𝐭𝐨 𝐠𝐞𝐭 𝐩𝐚𝐢𝐫𝐢𝐧𝐠 𝐜𝐨𝐝𝐞. 𝐏𝐥𝐞𝐚𝐬𝐞 𝐜𝐡𝐞𝐜𝐤 𝐲𝐨𝐮𝐫 𝐩𝐡𝐨𝐧𝐞 𝐧𝐮𝐦𝐛𝐞𝐫 𝐚𝐧𝐝 𝐭𝐫𝐲 𝐚𝐠𝐚𝐢𝐧.' });
                    }
                }
            }
